...
def train(model, dataloader, epochs=3, lr=1e-3):
    ...
    for epoch in range(epochs):
        ...
        for notes, others in dataloader:
            ...
            loss_vel = ce(vel_logits[:, :-1].reshape(-1, vel_logits.size(-1)),
                          others[:, 1:, 0].reshape(-1))

            loss_dt = ce(dt_logits[:, :-1].reshape(-1, dt_logits.size(-1)),
...
def compose(model, seed_notes, seed_others, length=100):
    ...
    for _ in range(length):
        ...
        pc_logits, oct_logits, vel_logits, dt_logits, hidden = model.step(
            note, vel, dt, hidden
        )

        # sample pitch components
        pc = sample(pc_logits)
        octv = sample(oct_logits)

        next_note = octv * 12 + pc

        # sample rhythm/velocity
        next_vel = sample(vel_logits)
        next_dt  = sample(dt_logits)
